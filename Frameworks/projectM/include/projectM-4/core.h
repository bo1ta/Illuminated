 ...
 *
 * If this function returns NULL, in most cases the OpenGL context is not
 * initialized, not made current or insufficient to render projectM visuals.
 *
 * @return A projectM handle for the newly created instance that must be used in
 * subsequent API calls. NULL if the instance could not be created successfully.
 */
 ...
 *
 * After destroying the handle, it must not be used for any other calls to the
 * API.
 *
 * @param instance A handle returned by projectm_create() or
 * projectm_create_settings().
 */
 ...
 *
 * Ideally, the filename should be given as a standard local path. projectM also
 * supports loading "file://" URLs. Additionally, the special filename "idle://"
 * can be used to load the default idle preset, displaying the "M" logo.
 *
 * Other URL schemas aren't supported and will cause a loading error.
 *
 * If the preset can't be loaded, no switch takes place and the current preset
 * will continue to be displayed. Note that if there's a transition in progress
 * when calling this function, the transition will be finished immediately, even
 * if the new preset can't be loaded.
 *
 ...
 */
PROJECTM_EXPORT void projectm_load_preset_file(projectm_handle instance,
                                               const char *filename,
 ...
 * @param smooth_transition If true, the new preset is smoothly blended over.
 */
PROJECTM_EXPORT void projectm_load_preset_data(projectm_handle instance,
                                               const char *data,
                                               bool smooth_transition);
 ...
 */
PROJECTM_EXPORT void projectm_get_version_components(int *major, int *minor,
                                                     int *patch);

/**
 * @brief Returns the runtime library version as a string.
 *
 * Remember to call  @a projectm_free_string() on the returned pointer if the
 ...
 *
 * Can be any text, will mostly contain a Git commit hash. Useful to report
 * bugs.
 *
 * Remember to call  @a projectm_free_string() on the returned pointer if the
 * data is no longer needed.
