...
+ (BFTask<FileBrowserItem *> *)createFileBrowserItemWithURL:(NSURL *)url {
  ...
  FileBrowserItem *fileBrowserItem = [self newFileBrowserItemWithURL:url bookmarkData:bookmarkData];
  return [[FileBrowserLocationDataStore createFromFileBrowserItem:fileBrowserItem]
          continueWithBlock:^id(BFTask<FileBrowserLocation *> *task) {
    if (task.error) {
      NSLog(@"Error creating file browser location: %@", task.error.localizedDescription);
      return nil;
    } else {
