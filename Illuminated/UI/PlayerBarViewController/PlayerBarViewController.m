...
- (void)setupBindings {
    ...
    [self.volumeSlider bind:NSValueBinding
                   toObject:manager
                withKeyPath:@"volume"
                    options:nil];
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([keyPath isEqualToString:@"isPlaying"]) {
          ...
          self.artistName.stringValue = [[AppPlaybackManager sharedManager] currentStreamTitle] ?: self.artistName.stringValue;
        }
    });
}
...
- (void)updatePlayPauseButton {
    ...
    self.playPauseButton.image = [NSImage imageWithSystemSymbolName:imageName
...
- (void)loadWaveformForCurrentTrack {
    ...
    [[TrackService getWaveformForTrack:track
                           resolvedURL:url
                                  size:self.waveformView.bounds.size] continueOnMainThreadWithBlock:^id(BFTask<NSImage *> *task) {
            if (task.result) {
