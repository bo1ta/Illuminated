...
/*!
 ...
 @note This function cannot be inlined, as otherwise the internal implementation could fail to
 report the proper remaining stack space.
 */
...
+ (instancetype)defaultExecutor {
  ...
  dispatch_once(&onceToken, ^{
    defaultExecutor = [self executorWithBlock:^void(void (^block)(void)) {
      ...
      // when debugging. However, we don't want the stack to get too deep, so if the remaining stack
      // space is less than 10% of the total space, we dispatch to another GCD queue.
