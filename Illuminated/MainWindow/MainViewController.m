...
- (void)loadArtists {
    [[[CoreDataStore readOnlyStore] allArtists] continueWithBlock:^id _Nullable(BFTask<NSArray<Artist *> *> * _Nonnull task) {
        if (task.error) {
   ...
   viewForTableColumn:(NSTableColumn *)tableColumn
    ...
    if (!cell) {
        ...
        [cell addConstraints:[NSLayoutConstraint
                              constraintsWithVisualFormat:@"H:|-8-[textField]-8-|"
                              options:0 metrics:nil views:views]];
        [cell addConstraints:[NSLayoutConstraint
                              constraintsWithVisualFormat:@"V:|-6-[textField]-6-|"
                              options:0 metrics:nil views:views]];
    }
