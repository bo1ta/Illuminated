...
- (BFTask<NSString *> *)fetchAuthToken {
  ...
  return [[self POST:@"" parameters:params] continueWithSuccessBlock:^id _Nullable(BFTask * _Nonnull task) {
    ...
    return [BFTask taskWithError:[NSError errorWithDomain:@"LastFMClient" code:-100 userInfo:@{NSLocalizedDescriptionKey : @"Failed to parse token response"}]];
  }];
}
...
//    @"format": @"json"
  ...
  return [[self POST:@"" parameters:params] continueWithSuccessBlock:^id _Nullable(BFTask * _Nonnull task) {
    ...
    if (!name || !key || [isSubscriber isKindOfClass:[NSNull class]]) {
      return [BFTask taskWithError:[NSError errorWithDomain:@"LastFMClient" code:-100 userInfo:@{NSLocalizedDescriptionKey : @"LastFM FetchSession response could not be parsed"}]];
    }
...
- (nullable NSString *)generateAPISignatureWithParams:(NSDictionary<NSString *, NSString *> *)params secret:(NSString *)secret {
